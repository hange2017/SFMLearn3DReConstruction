...
public:
    ...
    math::Vec2d projection(math::Vec3d const & p3d){
         ...
         *
         */
        //按照相机位姿将世界坐标系转换到相机坐标系下
        math::Matrix<double,3,1> p3d_cam;
        p3d_cam[0] = R_[0]*p3d[0]+R_[1]*p3d[1]+ R_[2]*p3d[2]+t_[0];
        p3d_cam[1] = R_[3]*p3d[0]+R_[4]*p3d[1]+ R_[5]*p3d[2]+t_[1];
        p3d_cam[2] = R_[6]*p3d[0]+R_[7]*p3d[1]+ R_[8]*p3d[2]+t_[2];

        //从相机坐标系到归一化平面
        double x=p3d_cam[0]/p3d_cam[2];
        double y=p3d_cam[1]/p3d_cam[2];

        //添加径向畸变模型
        double dist_r2 = x*x+y*y;
        double x_dist=(1+dist_[0]*dist_r2+dist_[1]*dist_r2*dist_r2)*x;
        double y_dist=(1+dist_[0]*dist_r2+dist_[1]*dist_r2*dist_r2)*y;

        //通过内参矩阵获得像素坐标并将原点移动到图像左上角
        p[0] = f_*x_dist+c_[0];
        p[1] = f_*y_dist+c_[1];
